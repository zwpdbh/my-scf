...
/**This struct is a wrapper around the flann::KMeansIndexParams,
 ...
 * stage when building the k-means tree.
 * The algorithm to use for selecting the initial centers.
 * cd_index: how exploration is performed in hierachical kmeans tree.
 */
struct KMeansParams : public IndexParams {
    KMeansParams(int branching = 64,
                 int iterations = 11,
        ...
        // algorithm used for picking the initial cluster centers for kmeans
        // tree
...
public:
     ...
     * @param params parameters passed to the hierarchical k-means algorithm
     *
     * This method resenble the method of flann::index::index which
     * construcs a nearest neighbor search index for a give dataset.
     */
    KMeans(const Matrix<ElementType> &inputData,
           const IndexParams &params = KMeansParams(),
           Distance d = Distance())
    ...
    /**After calling this method, get all the clusters in an hierarchical
     * structure*/
    Cluster *getCluster() {
        ...
        // size_ is the number of points in the index
...
protected:
    /**
     * inherited method for building the index on
     */
    void buildIndexImpl() {
        if (branching_ < 2) {
...
private:
     ...
     *
     */
    void computeClustering(NodePtr node,
                           int *indices,
                           int indices_length,
                           int branching) {
        ...
        for (int i = 0; i < indices_length; ++i) {
            ...
            DistanceType sq_dist =
                    distance_(points_[indices[i]], dcenters[0], veclen_);
            belongs_to[i] = 0;
            for (int j = 1; j < branching; ++j) {
                DistanceType new_sq_dist =
                        distance_(points_[indices[i]], dcenters[j], veclen_);
        ...
        for (int c = 0; c < branching; ++c) {
            ...
            for (int i = 0; i < indices_length; ++i) {
                if (belongs_to[i] == c) {
                    float dist =
                            distance_(centers[c], points_[indices[i]], veclen_);
            ...
            computeClustering(node->childs[c],
                              indices + start,
                              end - start,
                              end - start + 10);
