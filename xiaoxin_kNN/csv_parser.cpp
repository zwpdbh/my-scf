...
bool csv_parser::init(FILE * input_file_pointer) {
	...
	if (input_fp == NULL) {
		fprintf(stderr,
				"Fatal error : unable to open input file from file pointer\n");
...
bool csv_parser::init(const char * input_file) {
	...
	if (input_filename == NULL) {
		fprintf(stderr,
				"Fatal error : unable to allocate memory for file name buffer %s\n",
				input_file);
	...
	printf("In csv_parser::init, try to open: %s\n", input_file);
	input_fp = fopen(input_file, "r");

	if (input_fp == NULL) {
		fprintf(stderr, "During csv_parser::init, Fatal error : unable to open input file %s \n",
				input_file);

		CSV_PARSER_FREE_BUFFER_PTR(input_filename);
		return false;
	} else {
	    printf("read csv_file:==> %s succeed\n", input_file);
	}
...
void csv_parser::_read_single_line(char ** buffer, unsigned int * buffer_len) {
	...
	if (length_of_row > 0) {
		...
		*buffer = (char *) realloc(*buffer, *buffer_len);
		...
		tmp = fread(*buffer, 1, length_of_row, input_fp);
	}
}
